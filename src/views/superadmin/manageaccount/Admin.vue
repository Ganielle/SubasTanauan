<template>
  <div>
    ...
    <br/><br/>
    <AdminTable v-if="!loadingapi" :useritems="userlist" @edit-user="toggleEditUser" @delete-user="toggleDeleteUser"/>
    <center v-else>
    ...
    <div>
      <vue-final-modal v-model="showModal" classes="modal-container" content-class="modal-content">
        ...
        <span class="modal__title">Create Admin Account</span>
        <br/>
  ...
  methods: {
    ...
    toggleDeleteUser(userid){
      ...
      this.$swal({
        ...
        if (data.isConfirmed){
          const response = await fetch(`${process.env.VUE_APP_API_URL}/deletestaff`, {
            method: 'POST',
            headers: {
    ...
    async listUsers() {
      ...
      const response = await fetch(`${process.env.VUE_APP_API_URL}/users/liststaffs?authfilter=admin&fullnamefilter=${this.search}&page=${this.pagination.page}&limit=${this.pagination.limit}`, {
        method: 'GET',
        headers: {
          "Content-Type": "application/json"
        },
